                ...
                <div>
                  ...
                  {userData.educations.map((education, index) => (
                    <div key={index}>
                      ...
                      <div style={{ display: "flex", gap: "4px" }}>
                        {education.degree && (
                          <p className='edu-info'>{`${education.degree}`}</p>
                        )}
                        {education.degree && education.fieldOfStudy && (
                          <p className='edu-info'>, </p>
                        )}
                        {education.fieldOfStudy && (
                          <p className='edu-info'>{`${education.fieldOfStudy}`}</p>
                        )}
                      </div>
                      <div>
                        {education.startDate && education.leaveDate && (
                          <p className='year-act'>{`${education.startDate} - ${education.endDate}  `}</p>
                        )}
                      </div>
                      <div>
                ...
                <div>
                  ...
                  {userData.companies.map((company, index) => (
                    <div key={index}>
                      ...
                      <div style={{ display: "flex", gap: "4px" }}>
                        {company.designation && (
                          <p className='edu-info'>{`${company.designation}`}</p>
                        )}
                        {company.designation && company.country && (
                          <p className='edu-info'>, </p>
                        )}
                        {company.country && (
                          <p className='edu-info'>{`${company.country}`}</p>
                        )}
                      </div>
                      <div>
                        {company.joinDate && company.leaveDate && (
                          <p className='year-act'>{`${company.joinDate} - ${company.leaveDate}  `}</p>
                        )}
                      </div>
                      <div style={{ marginTop: "10px" }}>
                        <p className='description-data'>{`${company.description} `}</p>
                      </div>
                      {index !== userData.companies.length - 1 && (
                        <hr
                          style={{ marginTop: "10px", marginBottom: "10px" }}
                        />
                      )}
